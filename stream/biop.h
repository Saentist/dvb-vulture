...
/* functions */
bool process_biop (uint16_t tsid, uint16_t current_pid, Carousel *,
                   struct module *, struct BIOPMessageHeader *, uint32_t);
void process_biop_dir (uint8_t, char *, Carousel *, unsigned char *,
                       uint32_t);
uint32_t process_iop_ior (uint8_t, unsigned char *, struct biop_iop_ior *);
uint16_t process_biop_service_gateway_info (uint16_t, uint16_t,
                                            struct assoc *, unsigned char *,
                                            uint16_t);
...
uint32_t biop_sequence65535 (uint8_t, unsigned char *,
                             struct biop_sequence *);
