...
bool
process_biop (uint16_t tsid, uint16_t current_pid, Carousel * car,
              struct module * mod, struct BIOPMessageHeader * data,
              uint32_t size)
{
  uint32_t bytes_left;
  ...
  while (bytes_left != 0)
  {
    ...
    if (strcmp ((char *) kind.data, BIOP_DIR) == 0)
    {
      ...
      dirname =
        make_dir ((char *) kind.data, tsid, current_pid, mod->download_id,
                  mod->module_id, (char *) key.data, key.size);
    ...
    else if (strcmp ((char *) kind.data, BIOP_FILE) == 0)
    {
      ...
      save_file ((char *) kind.data, tsid, current_pid, mod->download_id,
                 mod->module_id, (char *) key.data, key.size, file.data,
                 file.size);
    }
    else if (strcmp ((char *) kind.data, BIOP_STREAM) == 0)
    {
       ...
       */
      save_file ((char *) kind.data, tsid, current_pid, mod->download_id,
                 mod->module_id, (char *) key.data, key.size, body.data,
                 body.size);
    }
    ...
    /* move onto the next */
    data =
      (struct BIOPMessageHeader *) (((unsigned char *) data) +
                                    sizeof (struct BIOPMessageHeader) + size);
...
void
process_biop_dir (uint8_t byte_order, char *dirname, Carousel * car,
                  unsigned char *data, uint32_t NOTUSED size)
{
  uint16_t nbindings;
  uint16_t i;
  uint8_t nnames;
  ...
  struct biop_sequence info;
  uint16_t pid;
  kind.data = (unsigned char *) "k_invalid";
  kind.size = strlen ((char *) kind.data) + 1;
  name.data = (unsigned char *) "n_invalid";
  name.size = strlen ((char *) kind.data) + 1;
  ...
  for (i = 0; i < nbindings; i++)
  {
    ...
    pid = stream2pid (&car->backptr->assoc, ior.association_tag);
    /*
    ...
    add_dir_entry (dirname, (char *) name.data, (int) name.size,
                   (char *) kind.data, pid, ior.carousel_id, ior.module_id,
                   (char *) ior.key.data, ior.key.size);
    /* objectInfo */
...
uint16_t
process_biop_service_gateway_info (uint16_t tsid, uint16_t service_id,
                                   struct assoc * assoc, unsigned char *data,
                                   uint16_t size)
{
  struct biop_iop_ior ior;
  ...
  make_service_root (tsid, service_id, BIOP_SERVICEGATEWAY, elementary_pid,
                     ior.carousel_id, ior.module_id, (char *) ior.key.data,
                     ior.key.size);
...
uint32_t
process_iop_ior (uint8_t byte_order, unsigned char *data,
                 struct biop_iop_ior * ior)
{
  ...
  for (i = 0; i < nprofiles; i++)
  {
    ...
    if (tag == TAG_BIOP)
    {
      ...
      /* carouselId */
      ior->carousel_id =
        biop_uint32 (profile_bo, *((uint32_t *) profile.data));
      profile.data += 4;
      ...
      if (taps_count > 0)
      {
        ...
        profile.data += 2;
        if (biop_uint16 (profile_bo, *((uint16_t *) profile.data)) !=
            BIOP_DELIVERY_PARA_USE)
          fatal ("Expecting BIOP_DELIVERY_PARA_USE");
        profile.data += 2;
        vverbose ("    use: BIOP_DELIVERY_PARA_USE");
        ior->association_tag =
          biop_uint16 (profile_bo, *((uint16_t *) profile.data));
        profile.data += 2;
...
uint32_t
biop_sequence65535 (uint8_t byte_order, unsigned char *raw,
                    struct biop_sequence * out)
{
...
uint32_t
biop_sequence (uint8_t byte_order, unsigned char *raw,
               struct biop_sequence * out)
{
