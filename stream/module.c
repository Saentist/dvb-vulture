...
struct module *
find_module (Carousel * car, uint16_t module_id, uint8_t version,
...
struct module *
add_module (Carousel * car, struct DownloadInfoIndication *dii,
            struct DIIModule *diimod)
{
  struct module *mod;

  car->nmodules++;
  car->modules =
    safe_realloc (car->modules, car->nmodules * sizeof (struct module));
  ...
  mod->nblocks =
    (ntohl (diimod->moduleSize) + mod->block_size - 1) / mod->block_size;
...
//  mod->data = safe_malloc (mod->size);  //BUG? one of those got overwritten at the end..
  mod->data = safe_malloc (mod->block_size * mod->nblocks);

  verbose ("add_module: nmodules=%u module=%u size=%u", car->nmodules,
...
void
delete_module (Carousel * car, uint32_t num)
{
  uint32_t size;
...
void
download_block (uint16_t tsid, uint16_t current_pid, Carousel * car,
                struct module *mod, uint16_t block, unsigned char *data,
                uint32_t length)
{
  /* assert */
  if (block >= mod->nblocks)
  {
    error ("Error: download_block: moduleId=%u block=%u nblocks=%u",
           mod->module_id, block, mod->nblocks);
    return;
  }
  ...
  if (mod->got_block[block])
    return;
  if (length > mod->block_size)
  {
    error ("Error: download_block: moduleId=%u length=%u mod->block_size=%u",
           mod->module_id, length, mod->block_size);
    return;
  }
  ...
  verbose ("download_block: module=%u block=%u left=%u", mod->module_id,
           block, mod->blocks_left);
  ...
  if (mod->blocks_left == 0)
  {
    ...
    if (process_biop
        (tsid, current_pid, car, mod, (struct BIOPMessageHeader *) mod->data,
         mod->size))
    {
